...
class AppConfig:
    ...
    # 主动轮换阈值：每个 device_id 用 N 次后预换（0 关闭，默认 8）
    device_id_rotate_threshold: int
...
def load_config(env_file: str = ".env") -> AppConfig:
    ...
    config = AppConfig(
        ...
        request_timeout=parse_int(values.get("REQUEST_TIMEOUT_SECONDS"), 60),  # v47: 120→60s
        ...
        glm_queue_wait_timeout=parse_int(values.get("GLM_QUEUE_WAIT_TIMEOUT_SECONDS"), 60),  # v47: 600→60s，避免 10 分钟挂起
        glm_busy_max_retries=parse_int(values.get("GLM_BUSY_MAX_RETRIES"), 5),  # v47: 30→5，避免 60s 等待
        glm_busy_retry_interval=parse_float(values.get("GLM_BUSY_RETRY_INTERVAL_SECONDS"), 1.0),  # v47: 2.0→1.0
        glm_guest_max_retries=max(0, parse_int(values.get("GLM_GUEST_MAX_RETRIES"), 3)),
        # 主动轮换阈值：每个 device_id 用 N 次后预换（0=关闭）
        # 默认 8，平衡"避免风控"和"减少 token fetch 开销"
        device_id_rotate_threshold=max(0, parse_int(values.get("GLM_DEVICE_ID_ROTATE_THRESHOLD"), 8)),
