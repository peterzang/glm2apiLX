...
# v11 验证：GLM-5.2 支持 1M+ tokens 上下文
...
glm2api 通过 `GLM_MAX_CONCURRENCY` 配置最大并发数（默认 8，游客模式自动创建同等数量账号槽位）。

**并发性能实测**（v21）：
- 8 并发：8/8 成功（100%），3.2s 总耗时
- 16 并发：16/16 成功（100%），3.2s 总耗时（请求排队但都成功）
- 32 并发：25/32 成功（78%），7 个 Connection reset（超 MAX_CONCURRENCY=8 限制）

**配置建议**：
- 默认 8 并发适合大多数场景（codex CLI / Claude Code 单客户端）
- 如需更高并发，在 `.env` 中提高 `GLM_MAX_CONCURRENCY`（如 16/32/100）
- 游客模式上游可能有额外限制，超过 16 并发时偶发 Connection reset
- 生产环境建议用付费账号 + 提高 `GLM_MAX_CONCURRENCY` 到 32+

**超并发失败处理**：
- 超过 MAX_CONCURRENCY 的请求会被上游拒绝（Connection reset by peer）
- glm2api 正确处理这种错误（返回错误响应而非崩溃）
- 客户端可以重试失败请求
...
GLM-5.2 通过 glm2api 支持 **1M+ tokens 上下文**（v11 审核验证）：

| 上下文规模 | prompt_tokens | 耗时 | 回答正确？ |
|-----------|---------------|------|-----------|
| 10K tokens | 12,270 | ~4s | ✅ |
| 50K tokens | 61,270 | ~3s | ✅ |
| 100K tokens | 122,520 | ~4s | ✅ |
| 500K tokens | 612,520 | 2.6s | ✅ |
| 1M tokens | 1,225,020 | 3.8s | ⚠️ 注意力分散 |

glm2api **不截断不报错**，4.5M 字符的 prompt 完整转发给上游 chatglm.cn。

**推荐**：
- 500K tokens 以内：完美使用，准确率 100%
- 1M tokens：能处理但可能注意力分散，推荐拆分
- codex config 设置 `model_context_window = 1000000`
