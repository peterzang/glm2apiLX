...
**根因**：GLM-5.2 模型本身在生成复杂 shell 命令（含 heredoc + 嵌套引号 + f-string）时的转义能力不足，**不是 glm2api 项目的 bug**。

**规避方式**：
- codex 任务拆小，避免生成复杂 heredoc
- 用 `glm-5.1` 跑中等复杂度任务（shell 命令生成更稳定）
- 等上游 GLM 修复

### M5 codex 长任务稳定性仍不足

**现象**：codex 长 agentic 任务（多文件构建 + pytest 迭代）仍不可靠，3 次测试结果不稳定（8s 空退 / 30s 部分工具调用 / 84s 复读）。

**建议**：
- 把长任务拆成多个小 prompt 分别发请求
- 短任务（< 200 字符响应）稳定性 10/10
- 中等任务用 `glm-5.1`
- 长任务等 GLM-5.2 模型侧改进后再评估
